...
private void loadStockWatcher() {
	...
	//	twitterLabelResult.setText("please wait, checking login on server");
  ...
  RootPanel.get("twitterLoginLink").add(twitterLoginLink);   
  RootPanel.get("twitterLabelResult").add(twitterLabelResult);
  RootPanel.get("twitterLogoutButton").add(twitterLogoutButton);
...
private void twitterLogin(){
	...
	twitterService.login( new AsyncCallback<TwitterInfo>() {
		...
		public void onSuccess(TwitterInfo cred) {
			...
			if (cred == null) {
				// TODO handle error
				twitterLoginLink.setText( "An exception occurred on the server" );
			} 
			else if (cred.getUser() != null) {
				twitterLabelResult.setText( "You are currently logged into Twitter as " + 
					cred.getUser() );
				twitterLoginLink.setVisible( false );
...
private void displayStock(final String symbol) {
	...
	// Add a button to tweet stock
	Button tweetNoteButton = new Button("tweet");
	tweetNoteButton.addClickHandler(new ClickHandler(){
		public void onClick(ClickEvent event){
			tweetNote(symbol);
		}
	});
	stocksFlexTable.setWidget(row, 4, tweetNoteButton);
